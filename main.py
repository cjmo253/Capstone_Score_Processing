...
def deskew(img):
    ...
    coords = np.column_stack(np.where(
        skew_img > 0))  # grab the (x, y) coordinates of all pixel values that are greater than zero, then use these coordinates to compute a rotated bounding box that contains all coordinates
    angle = cv2.minAreaRect(coords)[
        -1]  # the `cv2.minAreaRect` function returns values in the range [-90, 0); as the rectangle rotates clockwise the returned angle trends to 0 --

    if angle < -45:  # in this special case we need to add 90 degrees to the angle
        angle = -(90 + angle)
    else:  # otherwise, just take the inverse of the angle to make
        angle = -angle
...
def get_ref_lengths(img):
    ...
    assert (
                line_spacing + line_width == width_spacing_sum), "Estimated Line Thickness + Spacing doesn't correspond with Most Common Sum "
    ...
    for i in range(len(all_staffline_vertical_indices)):
        ...
        for j in range(num_cols // 2):
            first_staff_rows_isolated = img[
                                        all_staffline_vertical_indices[i][0][0]:all_staffline_vertical_indices[i][4][
                                            line_width - 1], j]
...
if __name__ == "__main__":
    ...
    # try to make pdf a jpg
    #   m = str(sys.argv[1])
    #   if m.endswith('.pdf') :  # check if pdf is first input
    #       print('you added a pdf')
    #       images = convert_from_path(sys.argv[1])  # pdf to images
    #       for page in pages:
    ...
    # start loop to iterate through inputs
    num_songs = (len(sys.argv) - 1)
    # print(num_songs)
    t = -1
    midi = MIDIFile(2)  # 0 is single track. 1 is synchronous. 2 is separate, appended
    timenew = 0

    for i in range(0, num_songs):
        t += 1
        # Read Image correctly iterates through 2 images
        img_file = sys.argv[i + 1:][0]  # 1 is first song
        img = cv2.imread(img_file, 0)
        # test that image is read
        # imgplot = plt.imshow(img) #present
        # plt.show()
        # ============ Noise Removal ============
        ...
        # ("[INFO] Deskew Angle: {:.3f}".format(angle))
        ...
        # The projection with the highest local maxima is chosen.
        ...
        all_staffline_horizontal_indices = find_staffline_columns(img, all_staffline_vertical_indices, line_width,
                                                                  line_spacing)
        # print("[INFO] Found all staff line horizontal extremes")

        # ============ Show Detected Staffs ============
        staffs = []
        half_dist_between_staffs = (all_staffline_vertical_indices[1][0][0] - all_staffline_vertical_indices[0][4][
            line_width - 1]) // 2
        ...
        for i in range(len(all_staffline_vertical_indices)):
            ...
            staff_img = img[max(0, y - half_dist_between_staffs): min(y + height + half_dist_between_staffs,
                                                                      img.shape[0] - 1), x:x + width]
        ...
        # save #printed files with my own song for final #present
        ...
        for i in range(len(staffs)):
            ...
            quarter_boxes = locate_templates(staff_img, quarter_note_imgs, quarter_note_lower, quarter_note_upper,
                                             quarter_note_thresh)
            ...
            whole_boxes = locate_templates(staff_img, whole_note_imgs, whole_note_lower, whole_note_upper,
                                           whole_note_thresh)
            ...
            eighth_boxes = locate_templates(staff_img, eighth_rest_imgs, eighth_rest_lower, eighth_rest_upper,
                                            eighth_rest_thresh)
            ...
            quarter_boxes = locate_templates(staff_img, quarter_rest_imgs, quarter_rest_lower, quarter_rest_upper,
                                             quarter_rest_thresh)
            quarter_boxes = merge_boxes([j for i in quarter_boxes for j in i], 0.5)
            ...
            whole_boxes = locate_templates(staff_img, whole_rest_imgs, whole_rest_lower, whole_rest_upper,
                                           whole_rest_thresh)
            whole_boxes = merge_boxes([j for i in whole_boxes for j in i], 0.5)
            ...
            flag_boxes = locate_templates(staff_img, eighth_flag_imgs, eighth_flag_lower, eighth_flag_upper,
                                          eighth_flag_thresh)
            ...
            if (j != 0):
                ...
                max_accidental_offset_x = staff_primitives[j].getBox().getCenter()[0] - staff_primitives[
                    j].getBox().getWidth()
                ...
                for primitive in staff_primitives:
                    ...
                    if (type == "note" and note[0] in notes_to_modify):
                        new_note = MIDI_to_pitch[pitch_to_MIDI[note] + 1] if accidental_type == "sharp" else \
            ...
            for j in range(len(staff_primitives)):
                ...
                if (accidental_type == "flat" or accidental_type == "sharp"):
                    max_accidental_offset_x = staff_primitives[j + 1].getBox().getCenter()[0] - staff_primitives[
                        j + 1].getBox().getWidth()
        ...
        midi.addTrackName(t, timenew, str(t))  # init?
        midi.addTempo(t, timenew, 88)  # init?
            ...
            for j in range(len(bars)):
                ...
                for k in range(len(primitives)):
                    ...
                    if (primitives[k].getPrimitive() == "note"):
                        ...
                        midi.addNote(track, channel, pitch, timenew, duration, volume)
    ...
    binfile = open("C:\\Users\\chuml\\PycharmProjects\\MusicImage\\venv\\output\\speed.mid", 'wb')
    midi.writeFile(binfile)
    binfile.close()

# python main.py "C:\Users\chuml\OneDrive\Desktop\resources\samples\races.jpg"

# appends subsequent input files so 3 song inputs is one song with all 3 pages
